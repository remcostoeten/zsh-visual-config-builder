...
import { Book, Code, FileCode, GitFork, Terminal, HelpCircle } from 'lucide-react'
...
export default function ShellHelpers({ onInsertSnippet }: Props) {
    ...
    const categories = ['all', 'variable', 'loop', 'function', 'conditional', 'export', 'alias']
    ...
    const renderTemplateSection = () => (
        <div className='space-y-2'>
            <div
                className='group p-2 hover:bg-[#333] rounded-lg cursor-pointer'
                onClick={() => onInsertSnippet(devTemplate.content)}
            >
                <div className='flex items-center justify-between'>
                    <span className='text-white font-medium'>Developer Template</span>
                    <div className='flex items-center gap-2'>
                        <InfoTooltip content='Optimize your development workflow' />
                        <GitFork className='w-4 h-4 text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity' />
                    </div>
                </div>
                <p className='text-xs text-gray-400'>
                    A comprehensive setup for Git, Node.js, and Docker development.
                </p>
            </div>

            <div
                className='group p-2 hover:bg-[#333] rounded-lg cursor-pointer'
                onClick={() => onInsertSnippet(dockerTemplate.content)}
            >
                <div className='flex items-center justify-between'>
                    <span className='text-white font-medium'>Docker Template</span>
                    <div className='flex items-center gap-2'>
                ...
                <p className='text-xs text-gray-400'>
                    A boilerplate setup specialized in improving your local docker workflow.
                </p>
            </div>
        </div>
    )
