...
export default function TaskDialog({ open, onClose, onSave, initialData }: Props) {
  ...
  const [category, setCategory] = React.useState<Task['category']>(initialData?.category || 'feature');
  const [priority, setPriority] = React.useState<Task['priority']>(initialData?.priority || 'medium');
